...
דני דין"></textarea>
    ...
    renderSummaryModal(present, absent, wasArchived) {
        ...
        if (absent.length > 0) {
            ...
            const absentByGroup = absent.reduce((acc, student) => { acc[student.group] = (acc[student.group] || []).concat(student.name); return acc; }, {});
        ...
        if (present.length > 0) {
            ...
            const presentByGroup = present.reduce((acc, student) => { acc[student.group] = (acc[student.group] || []).concat(student.name); return acc; }, {});
